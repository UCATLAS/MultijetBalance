...
int main(int argc, char *argv[])
{
  ...
  while ((key = (TKey*)next() )){
    ...
    h_2D.push_back( h_recoilPt_PtBal );
  }
  cout << "numToys is " << h_2D.size() << endl;
  ...
  while( h_2D.at(0)->GetXaxis()->GetBinLowEdge(largestBin) >= upperEdge){
    largestBin--;
  }
  ...
  for( int iBin=reverseBinEdges.at(reverseBinEdges.size()-1); iBin > 0; --iBin){
    ...
    // Loop over all toys //
    for(unsigned int iH = 0; iH < h_2D.size(); ++iH){
      TH1D* h_proj = h_2D.at(iH)->ProjectionY("h_proj", iBin, reverseBinEdges.at(reverseBinEdges.size()-1), "ed" );
      m_BalFit->Fit(h_proj, 0); // Rebin histogram and fit
      //!! Need to check here if fit failed, and otherwise give the projection?
      meanValues.push_back(m_BalFit->GetMean());
  ...
  for(unsigned int i=0; i < reverseBinEdges.size(); ++i){
    newXbins[numBins-i] = h_2D.at(0)->GetXaxis()->GetBinUpEdge(reverseBinEdges.at(i));
